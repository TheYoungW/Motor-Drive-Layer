...
`speed=1～100` 是普通 PV 唯一公开的运动参数。100% 时 J1～J7 的速度硬上限为
`[180,180,180,225,225,225,225]°/s`，加速度硬上限为
`[450,450,900,900,900,900,900]°/s²`；比例 `s` 对速度按 `s` 缩放、对加速度按
`s²` 缩放。每周期 POS_VEL 的 `V` 由当前参考速度和真实跟踪误差动态得到，不再使用
用户设置的固定值。旧加速度 setter/getter 只为 ABI 符号兼容保留，SDK 不应继续暴露。
...
保持上次有效 P 并累计差值，达到一个量化级后再更新；精确终点始终强制发送，轨迹
duration 不缩短。POS_VEL 的内部 V 同时根据当前规划关节速度动态调整，并受产品上限
