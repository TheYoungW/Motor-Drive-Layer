...
`set_joint_mit_fast(q, speed_percent=100)` 用于高频遥操，用户传最新关节角和
`0..100` 的参考速度百分比；Runtime 内部固定使用
J1～J7 `Kp=[190,190,100,100,70,60,50]`、
`Kd=[4.55,4.50,2.50,2.50,0.70,0.60,0.50]`，100% 对应 5 rad/s 的位置参考步进，
50% 对应 2.5 rad/s，0% 保留最新目标但不推进参考角，同时固定 `dq=0`、
`tau_ff=0`。标准 MIT 不接收速度百分比；两个方法都不创建有限轨迹或 Motion ID。
