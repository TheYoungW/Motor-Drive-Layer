...
class Controller::PacingBus final : public CanBus {
 public:
  ...
  void wake_receive() override { inner_->wake_receive(); }
...
Controller::Controller(std::shared_ptr<CanBus> bus, std::string endpoint_label,
    ...
    : bus_(std::make_shared<PacingBus>(bus)),
      ...
      receive_thread_policy_(receive_thread) {
...
void Controller::poll_feedback_once() {
  synchronous_receive_waiters_.fetch_add(1, std::memory_order_acq_rel);
  std::lock_guard<std::mutex> recv_lock(recv_mutex_);
  synchronous_receive_waiters_.fetch_sub(1, std::memory_order_acq_rel);
  for (;;) {
...
void Controller::start_polling() {
  ...
  configure_worker(polling_thread_, receive_thread_policy_);
}
...
void Controller::polling_loop() {
  ...
  while (polling_active_.load(std::memory_order_acquire)) {
    if (synchronous_receive_waiters_.load(std::memory_order_acquire) != 0) {
    ...
    try {
      {
        ...
        for (std::size_t drained = 0; drained < kMaximumFramesPerDrain;
             ++drained) {
          const auto frame = bus_->receive_for(
              drained == 0 ? std::chrono::milliseconds(10)
                           : std::chrono::milliseconds(0));
          if (!frame.has_value()) break;
          // Motors may be added while the first blocking receive is waiting;
          // resolve the fixed controller table only after a frame arrives so
          // discovery feedback cannot be discarded against a stale snapshot.
          const auto motors = sorted_motors();
          for (const auto& motor : motors) {
            if (motor->accepts_frame(*frame)) {
              motor->process_feedback_frame(*frame);
...
void Controller::send_mit_batch(const std::vector<MitBatchCommand>& commands) {
  send_mit_batch(commands.data(), commands.size());
}

void Controller::send_mit_batch(const MitBatchCommand* commands,
                                std::size_t count) {
...
void Controller::send_pos_vel_batch(const std::vector<PosVelBatchCommand>& commands) {
  send_pos_vel_batch(commands.data(), commands.size());
}

void Controller::send_pos_vel_batch(const PosVelBatchCommand* commands,
                                    std::size_t count) {
  std::lock_guard<std::mutex> lifecycle_lock(lifecycle_mutex_);
  if (bus_closed_) throw std::runtime_error(endpoint_label_ + " is closed");
  for (std::size_t index = 0; index < count; ++index) {
...
class ControllerGroup::Impl {
 public:
  static constexpr std::size_t kMaximumBatchCommands = 32;
  struct MitBatch {
    std::array<MitBatchCommand, kMaximumBatchCommands> commands{};
    std::size_t count = 0;
  };
  struct PosVelBatch {
    std::array<PosVelBatchCommand, kMaximumBatchCommands> commands{};
    std::size_t count = 0;
  };

  explicit Impl(std::vector<Controller*> controllers, ThreadPolicy transmit_threads)
      : controllers_(std::move(controllers)),
        transmit_thread_policy_(transmit_threads) {
    ...
    try {
      for (std::size_t i = 0; i < controllers_.size(); ++i) {
        ...
        configure_worker(workers_.back(), transmit_thread_policy_);
      }
    } catch (...) {
      {
        std::lock_guard<std::mutex> lock(state_mutex_);
  ...
  void dispatch_mit(const std::vector<MitBatchCommand>& commands) {
    std::lock_guard<std::mutex> call_lock(call_mutex_);
    {
      std::lock_guard<std::mutex> state_lock(state_mutex_);
      for (auto& batch : mit_batches_) batch.count = 0;
      partition(commands, mit_batches_, "MIT");
  ...
  void dispatch_pos_vel(const std::vector<PosVelBatchCommand>& commands) {
    std::lock_guard<std::mutex> call_lock(call_mutex_);
    {
      std::lock_guard<std::mutex> state_lock(state_mutex_);
      for (auto& batch : pos_vel_batches_) batch.count = 0;
      partition(commands, pos_vel_batches_, "POS_VEL");
 ...
 private:
  ...
  template <typename Command, typename Batch>
  void partition(const std::vector<Command>& commands,
                 std::vector<Batch>& batches, const char* kind) {
    if (commands.size() > kMaximumBatchCommands) {
      throw std::invalid_argument(std::string(kind) +
                                  " batch exceeds fixed capacity");
    }
    std::array<const MotorHandle*, kMaximumBatchCommands> unique{};
    std::size_t unique_count = 0;
    for (const auto& command : commands) {
      if (!command.motor) {
        throw std::invalid_argument(std::string(kind) +
                                    " batch contains a null motor");
      }
      for (std::size_t i = 0; i < unique_count; ++i) {
        if (unique[i] == command.motor.get()) {
          throw std::invalid_argument(std::string(kind) +
              " batch contains a duplicate motor");
        }
      }
      unique[unique_count++] = command.motor.get();
      bool found = false;
      for (std::size_t i = 0; i < controllers_.size(); ++i) {
        if (!controllers_[i]->owns_motor(command.motor)) continue;
        auto& batch = batches[i];
        if (batch.count >= kMaximumBatchCommands) {
          throw std::invalid_argument(std::string(kind) +
                                      " controller batch is full");
        }
      ...
      if (!found) {
        throw std::invalid_argument(std::string(kind) +
                                    " batch contains a foreign motor");
      }
    }
  }
  ...
  void worker_loop(std::size_t index) {
    ...
    for (;;) {
      ...
      try {
        if (kind == TaskKind::Mit) {
          controllers_[index]->send_mit_batch(mit.commands.data(), mit.count);
        } else {
          controllers_[index]->send_pos_vel_batch(
              pos_vel.commands.data(), pos_vel.count);
        }
      } catch (const std::exception& err) {
        error = err.what();
      } catch (...) {
  ...
  ThreadPolicy transmit_thread_policy_;
  ...
  std::vector<MitBatch> mit_batches_;
  std::vector<PosVelBatch> pos_vel_batches_;
