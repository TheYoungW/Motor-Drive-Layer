...
// Control frames are fail-fast by default. A positive override is reserved
// for explicit maintenance/diagnostic processes and must not be set on the
// 500 Hz runtime service.
constexpr auto kDefaultSendTimeout = std::chrono::milliseconds(0);
...
SocketCanFdBus::SocketCanFdBus(int fd, std::string interface, bool enable_brs,
  ...
  if (wake_fd_ < 0) {
    const auto error = os_error("eventfd for SocketCAN receive wakeup failed");
    ::close(fd_);
    fd_ = -1;
    throw error;
  }
...
void SocketCanFdBus::send(const CanFrame& frame) {
  ...
  if (fd < 0) throw std::runtime_error("socketcanfd fd already closed");
  write_frame_with_timeout(fd, &raw, sizeof(raw), send_timeout_,
...
std::optional<CanFrame> SocketCanFdBus::receive_for(std::chrono::milliseconds timeout) {
  ...
  if (fd < 0 || wake_fd < 0) {
    throw std::runtime_error("socketcanfd fd already closed");
  }
  pollfd fds[2]{{fd, POLLIN, 0}, {wake_fd, POLLIN, 0}};
  const int rc = ::poll(fds, 2, static_cast<int>(timeout.count()));
  if (rc < 0) {
  ...
  if ((fds[1].revents & POLLIN) != 0) {
    eventfd_t value = 0;
    (void)::eventfd_read(wake_fd, &value);
    return std::nullopt;
  }
  if ((fds[0].revents & (POLLERR | POLLHUP | POLLNVAL)) != 0) {
    throw std::runtime_error("socketcanfd receive poll reported socket error");
  }
  canfd_frame raw{};
...
void SocketCanFdBus::wake_receive() {
  int wake_fd = -1;
  {
    std::lock_guard<std::mutex> lock(state_mutex_);
    wake_fd = wake_fd_;
  }
  if (wake_fd >= 0) (void)::eventfd_write(wake_fd, 1);
}
