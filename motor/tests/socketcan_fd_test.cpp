...
void verify_receive_wakeup_is_immediate() {
  int sockets[2] = {-1, -1};
  require(::socketpair(AF_UNIX, SOCK_DGRAM | SOCK_NONBLOCK, 0, sockets) == 0,
          "create receive socket pair");
  auto bus = damiao::SocketCanFdBusTestPeer::adopt_fd(
      sockets[0], std::chrono::milliseconds(0));
  sockets[0] = -1;
  std::atomic<bool> entered{false};
  std::thread receiver([&] {
    entered.store(true, std::memory_order_release);
    const auto frame = bus->receive_for(std::chrono::milliseconds(5000));
    require(!frame, "eventfd wake returns no CAN frame");
  });
  ...
  require(std::chrono::steady_clock::now() - started <
          ...
          "eventfd interrupts blocking CAN receive immediately");
  bus->shutdown();
  ::close(sockets[1]);
}
}  // namespace
...
int main() {
  ...
  verify_receive_wakeup_is_immediate();
