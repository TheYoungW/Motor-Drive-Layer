...
namespace {
void update_maximum(std::atomic<uint64_t>& destination, uint64_t candidate) {
...
void SafetyRuntime::worker_loop() {
  ...
  for (;;) {
    ...
    if (run_arm_control) {
      ...
      if (last_control_tick_ != Clock::time_point{}) {
        const auto period_ns = static_cast<uint64_t>(
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                now - last_control_tick_).count());
        update_maximum(maximum_control_period_ns_, period_ns);
        if (period_ns > static_cast<uint64_t>(control_period.count())) {
          control_overruns_.fetch_add(1, std::memory_order_relaxed);
        }
      }
      ...
      std::string error;
      const bool sent =
          run_arm_control_cycle(now, combine_mit_arm_and_gripper, error);
      const auto send_ns = static_cast<uint64_t>(
          std::chrono::duration_cast<std::chrono::nanoseconds>(
              Clock::now() - send_started).count());
      update_maximum(maximum_send_time_ns_, send_ns);
      if (!sent) {
