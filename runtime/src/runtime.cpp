...
std::string configure_control_worker(std::thread& worker,
                                     const ArticoreRuntimeConfig& config) {
  std::string warning;
  if (config.lock_memory && ::mlockall(MCL_CURRENT | MCL_FUTURE) != 0) {
    warning = "mlockall failed";
  }
  int cpu = config.control_cpu;
  if (cpu < 0) {
    const auto count = std::thread::hardware_concurrency();
    if (count > 0) cpu = static_cast<int>(count - 1);
  }
  if (cpu >= 0) {
    cpu_set_t set;
    CPU_ZERO(&set);
    ...
    const int result = pthread_setaffinity_np(
        worker.native_handle(), sizeof(set), &set);
    if (result != 0) {
      if (!warning.empty()) warning += "; ";
      warning += "control affinity failed: " + std::to_string(result);
    }
  }
  if (config.realtime && config.control_priority > 0) {
    sched_param parameters{};
    parameters.sched_priority = config.control_priority;
    const int result = pthread_setschedparam(
        worker.native_handle(), SCHED_FIFO, &parameters);
    if (result != 0) {
      if (!warning.empty()) warning += "; ";
      warning += "control SCHED_FIFO failed: " + std::to_string(result);
    }
  }
  return warning;
}
...
SafetyRuntime::SafetyRuntime(
  ...
  // 500 Hz is the native product cadence. A bounded environment override is
  // retained for controlled timing experiments and platform qualification.
  control_hz_ = 500;
  ...
  const auto scheduling_warning = configure_control_worker(worker_, config_);
  if (!scheduling_warning.empty()) safety_reason_ = scheduling_warning;
}
