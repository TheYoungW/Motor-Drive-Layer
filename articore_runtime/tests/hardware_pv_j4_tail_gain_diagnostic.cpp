...
constexpr std::size_t kLeftJ4 = 3;
constexpr std::size_t kRightJ4 = 10;
...
void write_gains(damiao::MotorHandle* motor, const PvGains& gains) {
  ...
  if (!gains_match(read_gains(motor), gains)) {
    throw std::runtime_error("J4 PV gain readback mismatch");
  }
}
...
void write_sample(std::ofstream& output, const Profile& profile,
  ...
  output << ',' << std::abs(state.q[kLeftJ4] - target[kLeftJ4])
         << ',' << std::abs(state.q[kRightJ4] - target[kRightJ4]) << '\n';
}
...
Result measure_move(articore::SafetyRuntime& runtime,
  ...
  while (Clock::now() < deadline) {
    ...
    const float left_error = std::abs(state.q[kLeftJ4] - target[kLeftJ4]);
    const float right_error = std::abs(state.q[kRightJ4] - target[kRightJ4]);
    ...
    result.left_peak_velocity = std::max(
        result.left_peak_velocity, std::abs(state.dq[kLeftJ4]));
    result.right_peak_velocity = std::max(
        result.right_peak_velocity, std::abs(state.dq[kRightJ4]));
    result.left_overshoot = std::max(
        result.left_overshoot,
        directed_overshoot(state.q[kLeftJ4], start[kLeftJ4], target[kLeftJ4]));
    result.right_overshoot = std::max(
        result.right_overshoot,
        directed_overshoot(state.q[kRightJ4], start[kRightJ4], target[kRightJ4]));
    if (std::abs(state.dq[kLeftJ4]) > kAbortVelocity ||
        std::abs(state.dq[kRightJ4]) > kAbortVelocity ||
        result.left_overshoot > kAbortOvershoot ||
        result.right_overshoot > kAbortOvershoot) {
      throw std::runtime_error("J4 gain trial exceeded movement safety limit");
    }
    const bool stable = left_error <= kPositionTolerance &&
        right_error <= kPositionTolerance &&
        std::abs(state.dq[kLeftJ4]) <= kVelocityTolerance &&
        std::abs(state.dq[kRightJ4]) <= kVelocityTolerance;
    if (stable) {
  ...
  if (result.elapsed_s <= 0.0) {
    throw std::runtime_error("J4 gain trial did not settle within 12 s");
  }

  float left_min = arrival.q[kLeftJ4];
  float left_max = arrival.q[kLeftJ4];
  float right_min = arrival.q[kRightJ4];
  float right_max = arrival.q[kRightJ4];
  const auto observe_deadline = Clock::now() + kPostArrivalObservation;
  while (Clock::now() < observe_deadline) {
    require_healthy(runtime);
    const auto state = read_snapshot(resources);
    left_min = std::min(left_min, state.q[kLeftJ4]);
    left_max = std::max(left_max, state.q[kLeftJ4]);
    right_min = std::min(right_min, state.q[kRightJ4]);
    right_max = std::max(right_max, state.q[kRightJ4]);
    if (record) {
      const double elapsed = std::chrono::duration<double>(
          Clock::now() - started).count();
  ...
  result.left_final_error = final.q[kLeftJ4] - target[kLeftJ4];
  result.right_final_error = final.q[kRightJ4] - target[kRightJ4];
  return result;
}
...
Product make_test_target(const articore::YunyiRuntimeResources& resources,
  ...
  Product target = home;
  for (const auto index : {kLeftJ4, kRightJ4}) {
    const auto& joint = resources.joints[index];
...
int main(int argc, char** argv) {
  if (argc != 2 || std::strcmp(
          argv[1], "--i-understand-both-j4-will-move-and-gains-change") != 0) {
    std::cerr << "Refusing hardware movement/register writes. Pass "
                 "--i-understand-both-j4-will-move-and-gains-change\n";
    return 2;
  }

  const std::string trace_path =
      "/tmp/articore_both_j4_tail_gain_tuning_phase8.csv";
  ...
  output << ",error_left_j4,error_right_j4\n" << std::setprecision(9);
  ...
  auto* left_j4 = resources.arm_motors[kLeftJ4];
  auto* right_j4 = resources.arm_motors[kRightJ4];
  ...
  try {
    ...
    original_left = read_gains(left_j4);
    original_right = read_gains(right_j4);
    have_original = true;
    ...
    for (std::size_t profile_index = 0;
      ...
      const PvGains right_profile{
          ...
          profile.kp_apr, profile.ki_apr};
      write_gains(left_j4, left_profile);
      write_gains(right_j4, right_profile);
      std::cout << "PROFILE label=" << profile.label
                << " KP_APR=" << profile.kp_apr
                << " KI_APR=" << profile.ki_apr
                << " left_delta=" << target[kLeftJ4] - home[kLeftJ4]
                << " right_delta=" << target[kRightJ4] - home[kRightJ4]
                << std::endl;
    ...
    write_gains(left_j4, original_left);
    write_gains(right_j4, original_right);
    std::cout << "RESTORED both_J4 gains=original persisted=false\n";
    runtime.disconnect();
    connected = false;
    ...
    if (have_original) {
      try {
        ...
        write_gains(left_j4, original_left);
        write_gains(right_j4, original_right);
        std::cerr << "GAINS_RESTORED both_J4\n";
